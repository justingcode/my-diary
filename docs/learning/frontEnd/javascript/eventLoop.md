...
- **微任务**和**宏任务**不在一个任务队列
  ...
  - 微任务的产生：当执行到`script`脚本的时候，js 引擎会为全局创建一个**执行上下文**，在该执行上下文中维护了一个**微任务队列**。当遇到**微任务**，就会把微任务回调放到**微任务队列**中，当所有的 js 代码都执行完比，在退出**执行上下文**之前回去检查改队列，有回调就执行，没有就退出**执行上下文**。这就是我们所说的**每个宏任务都有一个微任务队列**。
  ## 总结
...
- 微任务执行完毕，第一次循环结束
- 检查宏任务队列，里面存有是个定时器宏任务`macro1 macro2 macro3 macro4`,按定时器延迟时间得到执行顺序，即`Event Queue：macro2、macro4、macro3、macro1`,依次拿出放入执行栈末尾执行（**注：浏览器 event loop 的 MacroTask queue,即是宏任务队列在每次循环中只会读取一个任务**）
